...
폰트 이름이 영문이 아닐 때 이를 브라우저에서 바르게 표현하고, HTML에서 불러온 스타일을 제대로 랜더링하려면, 반드시 CSS 인코딩을 선언해야 합니다. HTML과 동일한 인코딩을 문서 첫 줄에 공백 없이 선언합니다. 파일을 저장할 때는 반드시 선언한 인코딩과 동일한 인코딩으로 저정합니다. 
